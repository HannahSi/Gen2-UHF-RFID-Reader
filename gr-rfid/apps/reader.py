...
class reader_top_block(gr.top_block):
  ...
  def __init__(self):
    ...
    self.adc_rate = 1e6                 # ADC rate (2MS/s complex samples)
...
#    self.usrp_address_sink   = "addr=192.168.10.2,recv_frame_size=256"
    ...
    self.usrp_address_source = "name=MyB200,recv_frame_size=512"
    self.usrp_address_sink   = "name=MyB200,recv_frame_size=512"
    ...
    self.connect((self.tag_decoder,0), self.file_sink_decoder) # (Do not comment this line)
